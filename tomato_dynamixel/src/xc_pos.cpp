...
uint16_t position_xc_write = 2048; // 0~4095
int16_t dir = 5;

int main(int argc, char ** argv)
{
    ros::init(argc, argv, "xc_pos");
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  ...
  while(ros::ok())
  {
    ...
    // position mode
    dxl_comm_result = packetHandler2->write4ByteTxRx(portHandler, DXL_XC_ID, ADDR_GOAL_POSITION_P2, position_xc_write, &dxl_error);
    if (dxl_comm_result == COMM_SUCCESS) {
      //ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", DXL_XC_ID, position_xc_write);
    } else {
    ...
    position_xc_write = position_xc_write + dir;
    if ((position_xc_write > 3000)|(position_xc_write  < 1000)) dir = -dir;
