...
int main(int argc, char ** argv)
{
  ...
  while(ros::ok())
  {
    ...
    if (dxl_comm_result == COMM_SUCCESS) {
      //ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", DXL_AX1_ID, position_ax_write);
    } else {
    ...
    if (dxl_comm_result == COMM_SUCCESS)
    {
      //ROS_INFO("getPosition : [ID:%d] -> [POSITION:%d]", DXL_AX1_ID, position_rax_ead);
    } else {
